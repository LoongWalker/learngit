...
static bool match_callsite_name(LLVMNode *callNode, const std::vector<std::string>& names, const std::vector<unsigned int>& callSiteLines)
{
    using namespace llvm;
...
//callSiteLines只有一个元素，现在只对一个目标函数调用点做为切片准则
//但以后也可以是对多个调用点（多个函数）
bool LLVMDependenceGraph::getCallSites(const std::vector<std::string>& names,
                                       const std::vector<unsigned int>& callSiteLines,
                                       std::set<LLVMNode *> *callsites)
{
    //getCallSites(names, callsites);
    std::set<LLVMNode *> nodes;
    getCallSites(names, &nodes);

    //for(auto callSite : *callsites)
    for(std::set<LLVMNode *>::iterator i = nodes.begin(); i!= nodes.end(); ++i)
    {
        llvm::CallInst* pInst = llvm::dyn_cast<llvm::CallInst>((*i)->getValue());
        if(pInst)
        {
            auto& Loc = pInst->getDebugLoc();
            unsigned int line = Loc.getLine();
            if(std::find(callSiteLines.begin(), callSiteLines.end(), line) != callSiteLines.end())
            {
                callsites->insert(*i);
                break;
            }
